...
def load_movielens_tf(config):
    ...
    postfix = "-ratings"
    ratings, info = tfds.load(
        config.dataset_base_name + postfix,
...
def split_train_valid_test_tf(ratings_tf, split_ratios):
    ...
    print(
        f"Splitting the dataset into train, validation and test sets with sizes: {split_ratios}"
    )

    # Shuffle the dataset
    ratings_tf = ratings_tf.shuffle(
        num_samples, seed=42, reshuffle_each_iteration=False
    )
...
if __name__ == "__main__":
    ...
    # Split the dataset
    train_ds, valid_ds, test_ds = split_train_valid_test_tf(
        ratings_tf, ConfigLf.split_ratios
    )
